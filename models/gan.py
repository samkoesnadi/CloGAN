...
def raw_model_binaryXE_gan():
    ...
    feature_vectors_1 = tf.keras.layers.Conv2D(NUM_CLASSES, kernel_size=1, name="predictions_1",
                                               kernel_initializer=KERNEL_INITIALIZER, use_bias=False)(image_feature_vectors)

    feature_vectors_2 = tf.keras.layers.Conv2D(NUM_CLASSES, kernel_size=1, name="predictions_2",
                                               kernel_initializer=KERNEL_INITIALIZER, use_bias=False)(image_feature_vectors)

    feature_vectors = tf.keras.layers.Add()([feature_vectors_1, feature_vectors_2])
    feature_vectors = tf.keras.layers.Activation("sigmoid")(feature_vectors)

    output_layer = tf.keras.layers.GlobalAveragePooling2D()(feature_vectors)
    output_layer = tf.keras.layers.Activation("linear", name='predictions')(output_layer)
