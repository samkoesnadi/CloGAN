...
FEATURES_NP_FILE = "../records/train_input_features"
...
def read_filepath_dataset(filename, dataset_path):
    ...
    dataset = dataset.map(lambda data: read_resize_image(tf.strings.join([dataset_path, '/', data["image_path"]]), NUM_LEVELS),
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(TRAIN_N)):
        for j in tqdm(range(TRAIN_N)):
            if i==j: continue
            kernel_wasserstein_distance(features_nps[i], features_nps[j])